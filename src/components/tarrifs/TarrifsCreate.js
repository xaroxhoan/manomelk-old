...
const TarrifsCreate = ({ onSaved, onCancel }) => {
  ...
  const validateFields = async _ => {
    const errs = await validate(state, {
      title: [{name: 'required'}],
      price: [{name: 'required'}, {name: 'number', min: 1000}],
  ...
  const onSave = async () => {
    try {
      ...
      const response = await tarrifs.store({
        title: state.title,
